...
register_google(key = readLines(con <- file("api.key")))
#register_google(key = "")
mapa <- get_map(location = c(lon = min(EMPRESAS_GRUPO$GEOCODE$lon, na.rm = TRUE), lat = max(EMPRESAS_GRUPO$GEOCODE$lat, na.rm = TRUE)), zoom = 5, source = "google", scale = 2, force = TRUE, maptype = "roadmap")

#mapa <- get_map(location = c(lat = -25, lon = -48), zoom = 6, source = "google", scale = 1, force = TRUE, maptype = "roadmap")

ggmap(mapa) + geom_point(data = EMPRESAS_GRUPO, aes(x = EMPRESAS_GRUPO$GEOCODE[1], y = EMPRESAS_GRUPO$GEOCODE[2]), color="blue", alpha=0.5)
