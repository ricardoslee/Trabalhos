...
while (i < length(EMPRESAS_GRUPO$ID)) {
  ...
  EMPRESAS_GRUPO$GEOCODE[i] <- geocode(EMPRESAS_GRUPO$CEP[i], source = "google")
  Sys.sleep(1)
  i <- i + 1
}

mapa <- get_map(location = "Sao Paulo Brazil", zoom = 4, source = "google")

EMPRESAS_geocode <- EMPRESAS_GRUPO$GEOCODE

ggmap(mapa) + geom_point(data = EMPRESAS_GRUPO, aes(x = EMPRESAS_geocode$lon, y = EMPRESAS_geocode$lat, size = EMPRESAS_GRUPO$`Faturamento 2017`), color="red", alpha=0.5)
```
